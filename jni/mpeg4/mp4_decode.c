...
static int mp4_updatesize(mp4_decode* dec)
{
	if (dec->Codec.IDCT.Width > (MB_X-1)*16 || dec->Codec.IDCT.Height > (MB_Y-1)*16)
	{
		...
		dec->iserror = 1;
		dec->err_code= ERR_NOT_SUPPORTED;
...
static int mp4_updateinput( mp4_decode* dec )
{
	...
	if (dec->Codec.In.Format.Type == PACKET_VIDEO)
	{
		...
		result = CodecIDCTSetFormat(&dec->Codec,PF_YUV420,dec->Codec.In.Format.Format.Video.Width,dec->Codec.In.Format.Format.Video.Height,
									 		              IDCTWidth,IDCTHeight,dec->Codec.In.Format.Format.Video.Aspect);
		...
		if (result==ERR_NONE && dec->Codec.Node.Class != MSMPEG4_ID)
		{
			...
			if (dec->Codec.In.Format.ExtraLength > 0)
			{
				...
				dec->ignoresize = (char)(dec->Codec.In.Format.Format.Video.Width*dec->Codec.In.Format.Format.Video.Height > 0);
				dec->gethdr(dec);
				dec->ignoresize = 0;
				if (dec->validvol)
					dec->needvol = 0;
