...
typedef struct {
     QDM2Complex complex[256+1];
	 QDM2Complex onecomplex[MPA_MAX_CHANNELS][256];
} QDM2FFT;
...
static void qdm2_fft_tone_synthesizer (QDM2Context *q, int sub_packet)
{
    ...
    for (ch = 0; ch < q->channels; ch++) {
        memset(q->fft.onecomplex[ch], 0, q->fft_size * sizeof(QDM2Complex));
    }
    ...
    if (q->fft_coefs_min_index[4] >= 0)
        for (i = q->fft_coefs_min_index[4]; i < q->fft_coefs_max_index[4]; i++) {
            ...
            q->fft.onecomplex[ch][q->fft_coefs[i].offset + 0].re += c.re;
            q->fft.onecomplex[ch][q->fft_coefs[i].offset + 0].im += c.im;
            q->fft.onecomplex[ch][q->fft_coefs[i].offset + 1].re -= c.re;
            q->fft.onecomplex[ch][q->fft_coefs[i].offset + 1].im -= c.im;
        }
        ...
        if (q->fft_coefs_min_index[i] >= 0) {
            for (j = q->fft_coefs_min_index[i]; j < q->fft_coefs_max_index[i]; j++) {
                ...
                if (offset < q->frequency_range) {
                    ...
                    tone.complex = &q->fft.onecomplex[ch][offset];
...
static void qdm2_calculate_fft (QDM2Context *q, int channel, int sub_packet)
{
    ...
    for (i=1; i < n2; i++) {
        ...
        c = q->exptab[i].re;
        s = -q->exptab[i].im;
		f0 = (q->fft.onecomplex[channel][i].re - q->fft.onecomplex[channel][j].re) * gain;
        f1 = (q->fft.onecomplex[channel][i].im + q->fft.onecomplex[channel][j].im) * gain;
        f2 = (q->fft.onecomplex[channel][i].re + q->fft.onecomplex[channel][j].re) * gain;
		f3 = (q->fft.onecomplex[channel][i].im - q->fft.onecomplex[channel][j].im) * gain;
    ...
    q->fft.complex[ 0].re =  q->fft.onecomplex[channel][ 0].re * gain * 2.0;
    q->fft.complex[ 0].im =  q->fft.onecomplex[channel][ 0].re * gain * 2.0;
    q->fft.complex[n2].re =  q->fft.onecomplex[channel][n2].re * gain * 2.0;
    q->fft.complex[n2].im = -q->fft.onecomplex[channel][n2].im * gain * 2.0;
...
#if 0
	fwrite(q->fft.complex,sizeof(QDM2Complex),q->fft_size,dumpf);
	fflush(dumpf);
#endif
}
...
#if 0
	dumpf = fopen("e:\\tcpmp-qdm2.dump", "wb");
#endif
}
...
//  dump_context(q);
	av_log(NULL, AV_LOG_INFO, "q->sub_packet (%d)\n", q->sub_packet);
#if 0
	if(q->sub_packet == 15)
		q->sub_packet = q->sub_packet;
	if(q->sub_packet > 9)
		q->sub_packet = q->sub_packet;
	if(q->sub_packet == 0)
		q->sub_packet = q->sub_packet;
