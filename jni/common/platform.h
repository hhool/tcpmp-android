...
DLL int   		AllocEnvArray();
DLL void 		FreeEnvArray();
DLL ThreadEnv* 	ThreadEnvArray(int n);
DLL int 		SaveThreadEnv();
DLL int 		RemoveThreadEnv();
DLL ThreadEnv* 	GetThreadEnv(int ThreadID);
DLL void  		SetJVM(void *jvm);
DLL void* 		GetJVM();
DLL int 		GetThreadEnvCount();
extern const char g_szIniFile[MAXPATH];
#endif
void Platform_Init();
void Platform_Done();
void Log_Done();
