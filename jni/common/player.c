...
static void StopLoadMode(player_base* p)
{
	if (p->LoadMode)
	{
		DEBUG_MSG(DEBUG_PLAYER,T("LoadMode Leaving"));
...
static int SetPlay(player_base* p,bool_t Stop)
{
	...
	DEBUG_MSG2(DEBUG_PLAYER,T("player_base::SetPlay Play %d,%d"),p->Play,p->FFwd);
	...
	{
		DEBUG_MSG(DEBUG_PLAYER,T("player_base::SetPlay if (p->Format && p->Input && p->Timer)"));
		...
		DEBUG_MSG(DEBUG_PLAYER,T("player_base::SetPlay if (p->TimerPlay)"));
		if (p->TimerPlay)
		{
			DEBUG_MSG(DEBUG_PLAYER,T("player_base::SetPlay if (p->TimerPlay):UpdateKeepAlive"));
			UpdateKeepAlive(p);
			DEBUG_MSG(DEBUG_PLAYER,T("player_base::SetPlay if (p->TimerPlay):UpdatePlay"));
			UpdatePlay(p);
			DEBUG_MSG(DEBUG_PLAYER,T("player_base::SetPlay if (p->TimerPlay):UpdateSpeed"));
			UpdateSpeed(p);
		}
		else
		{
		...
		DEBUG_MSG(DEBUG_PLAYER,T("player_base::SetPlay UpdateRunProcess(p)"));
		UpdateRunProcess(p);
...
static bool_t TryPin(player_base* p,node* From,int FromNo,int ToClass,int* FoundFirst,int Failed)
{
	...
	DEBUG_MSG5(DEBUG_PLAYER,T("player_base::TryPin::GetNode::FromClass %d,TAG:%c,%c,%c,%c"),
					From->Class,\
					(From->Class&0x000000ff),
					(From->Class&0x0000ff00)>>8,\
					(From->Class&0x00ff0000)>>16,\
					(From->Class&0xff000000)>>24);

	DEBUG_MSG5(DEBUG_PLAYER,T("player_base::TryPin::GetNode::ToClass %d,TAG:%c,%c,%c,%c"),
					ToClass,\
					(ToClass&0x000000ff),
					(ToClass&0x0000ff00)>>8,\
					(ToClass&0x00ff0000)>>16,\
					(ToClass&0xff000000)>>24);
#endif
	...
	if (To)
	{
		...
		int Result;
#if defined(LOG)||defined(DEBUG)
		DEBUG_MSG5(DEBUG_PLAYER,T("player_base::TryPin::ToClass %d,TAG:%c,%c,%c,%c EXSIT"),
					To->Class,\
					(To->Class&0x000000ff),
					(To->Class&0x0000ff00)>>8,\
					(To->Class&0x00ff0000)>>16,\
					(To->Class&0xff000000)>>24);
#endif
		for (No=0;NodeEnum(To,No,&ToDef)==ERR_NONE;++No)
			if (ToDef.Type == TYPE_PACKET && (ToDef.Flags & DF_INPUT) && !(ToDef.Flags & DF_RDONLY))
			{
				...
				{
#if defined(LOG)||defined(DEBUG)
					DEBUG_MSG(DEBUG_PLAYER,T("player_base::TryPin::SUCCESSED"));
#endif
					if (FoundFirst)
						*FoundFirst = 1;
...
#if defined(LOG)||defined(DEBUG)
				DEBUG_MSG(DEBUG_PLAYER,T("player_base::TryPin::Disconnect:FAILED"));
#endif
...
static bool_t UpdateOutput(player_base* p,node* From,const datadef* FromDef,int* FoundFirst)
{
	...
	case TYPE_PACKET:
		...
		if (From->Get(From,FromDef->No|PIN_FORMAT,&Format,sizeof(Format))==ERR_NONE)
		{
			...
			{
				DEBUG_MSG5(DEBUG_PLAYER,T("player_base::UpdateOutput::From::Class:PACKET_VIDEO %d,TAG:%c,%c,%c,%c "),
					From->Class,\
					(From->Class&0x000000ff),
					(From->Class&0x0000ff00)>>8,\
					(From->Class&0x00ff0000)>>16,\
				...
				DEBUG_MSG5(DEBUG_PLAYER,T("player_base::UpdateOutput::From::Class:VOutputId:PACKET_VIDEO %d,TAG:%c,%c,%c,%c"),
					p->VOutputId,\
					(p->VOutputId&0x000000ff),
					(p->VOutputId&0x0000ff00)>>8,\
					(p->VOutputId&0x00ff0000)>>16,\
					(p->VOutputId&0xff000000)>>24);
			...
			{
				DEBUG_MSG5(DEBUG_PLAYER,T("player_base::UpdateOutput::From::Class:PACKET_AUDIO %d,TAG:%c,%c,%c,%c"),
					From->Class,\
					(From->Class&0x000000ff),
					(From->Class&0x0000ff00)>>8,\
					(From->Class&0x00ff0000)>>16,\
				...
				DEBUG_MSG5(DEBUG_PLAYER,T("player_base::UpdateOutput::From::Class:AOutputId:PACKET_AUDIO %d,TAG:%c,%c,%c,%c"),
					p->AOutputId,\
					(p->AOutputId&0x000000ff),
					(p->AOutputId&0x0000ff00)>>8,\
					(p->AOutputId&0x00ff0000)>>16,\
					(p->AOutputId&0xff000000)>>24);
			...
			DEBUG_MSG1(DEBUG_PLAYER,T("player_base::UpdateOutput PacketFormatEnumClass Count%d"),ARRAYCOUNT(List,int));
			for (i=ARRAYBEGIN(List,int);i!=ARRAYEND(List,int);++i)
...
static void Unlock(player_base* p, bool_t Input)
{
	...
	LockLeave(p->Lock);
	DEBUG_MSG(DEBUG_PLAYER,T("UnLocked"));
#endif
}
...
static void Lock(player_base* p, bool_t Input)
{
	...
	DEBUG_MSG(DEBUG_PLAYER,T("Locked"));
#endif
}
...
static int UpdateWnd(player_base* p,void* Wnd)
{
	if (Wnd != p->Wnd)
	{
		...
		else
		{
			...
			DEBUG_MSG(DEBUG_SYS,T("EventSet(p->EventRunProcess);"));
			EventSet(p->EventRunProcess);
			DEBUG_MSG(DEBUG_SYS,T("ThreadTerminate(p->ProcessThread);"));
			ThreadTerminate(p->ProcessThread);
			DEBUG_MSG(DEBUG_SYS,T("ThreadTerminate(p->ProcessThread); end"));
			p->ProcessThread = NULL;
			DEBUG_MSG(DEBUG_SYS,T("p->ProcessThread = NULL"));

			p->RunInput = 0;
			DEBUG_MSG(DEBUG_SYS,T("EventSet(p->InputThread);"));
			ThreadPriority(p->InputThread,0);
			DEBUG_MSG(DEBUG_SYS,T("EventSet(p->EventProcess);"));
			EventSet(p->EventProcess);
			DEBUG_MSG(DEBUG_SYS,T("EventSet(p->EventRunInput);"));
			EventSet(p->EventRunInput);
			DEBUG_MSG(DEBUG_SYS,T("ThreadTerminate(p->InputThread);"));
...
static int ProcessThread(player_base* p)
{
	...
	DEBUG_MSG1(DEBUG_SYS,T("player_base::ProcessThread::Notify(p,PLAYER_THREAD_JVM,1),ThreadId 0x%x"),ThreadId());
	//Notify(p,PLAYER_THREAD_JVM,1);
	SaveThreadEnv();
#endif
	while (p->Wnd)
	{
		...
		if (p->RunProcess)
		{
				...
				|| (Result == ERR_NEED_MORE_DATA && (p->NoMoreInput || State.BufferUsedAfter >= p->CurrBufferSize2-2))))
			{
				...
				p->Format->Set(p->Format,FORMAT_LOAD_SUB,NULL,NULL);
/*#if defined(TARGET_ANDROID)
				Notify(p,PLAYER_SYNCED,0);
#endif*/ //move down£¬this will deadlock
				p->Fill = 0;
				UpdatePlay(p);
				UpdateRunProcess(p);
			}
			...
			case ERR_END_OF_FILE:
				...
				{
					DEBUG_MSG(DEBUG_PLAYER,T("player_base::ProcessThread::ERR_END_OF_FILE"));
					Lock(p,1);
...
#if defined(TARGET_ANDROID)
						DEBUG_MSG1(DEBUG_PLAYER,T("ProcessThread::ERR_END_OF_FILE:p->bHasError:%d"),p->bHasError);
						if(p->bHasError==0)
						{
							Notify(p,PLAYER_PLAY_COMPLETE,1);
						}
#else
			...
			case ERR_NEED_MORE_DATA:
				DEBUG_MSG(DEBUG_PLAYER,T("player_base::ProcessThread::Process Needs More Data Sleep"));
			...
			case ERR_BUFFER_FULL: // no packets processed, we should wait
				DEBUG_MSG1(DEBUG_PLAYER,T("player_base::ProcessThread::ERR_BUFFER_FULL:Before ThreadSleep(4):%d"),GetTimeTick());

#ifdef MULTITHREAD
				ThreadSleep(4); // this should be more intelligent...
				DEBUG_MSG1(DEBUG_PLAYER,T("player_base::ProcessThread::ERR_BUFFER_FULL:End ThreadSleep(4):%d"),GetTimeTick());
#else
				if (!p->WaitForProcess)
		...
		else
		{
			...
			p->ProcessLocked = 0;
#endif
			DEBUG_MSG(DEBUG_PLAYER,T("player_base::ProcessThread::before EventWait(p->EventRunProcess,-1,NULL)"));
			EventWait(p->EventRunProcess,-1,NULL);
			DEBUG_MSG(DEBUG_PLAYER,T("player_base::ProcessThread::after EventWait(p->EventRunProcess,-1,NULL)"));
		}
	}
#if defined(TARGET_ANDROID)
	DEBUG_MSG1(DEBUG_SYS,T("player_base::ProcessThread::Notify(p,PLAYER_THREAD_JVM,0),ThreadId 0x%x"),ThreadId());
	//Notify(p,PLAYER_THREAD_JVM,0);
	RemoveThreadEnv();
#endif
	SAFE_END
	return 0;
...
static int InputThread(player_base* p)
{
	...
	SAFE_BEGIN
#if defined(TARGET_ANDROID)
	DEBUG_MSG1(DEBUG_SYS,T("player_base::InputThread::Notify(p,PLAYER_THREAD_JVM,1),ThreadId 0x%x"),ThreadId());
	//Notify(p,PLAYER_THREAD_JVM,1);
	SaveThreadEnv();
#endif
	p->IOError = 0;
	...
	if (!p->WaitForProcess)
		...
		if (p->RunInput)
		{
			...
			case ERR_BUFFER_FULL:
				...
				if (p->MicroDrive && p->BufferMax==p->CurrBufferSize2)
				{
					...
					DEBUG_MSG1(DEBUG_PLAYER,T("MicroDrive Full: new max:%d"),p->BufferMax);
				}
...
#ifdef MULTITHREAD
				DEBUG_MSG1(DEBUG_PLAYER,T("player_base::InputThread::EventWait(p->EventProcess,p->RunProcess %d ?2000:-1,NULL):before"),p->RunProcess);
				EventWait(p->EventProcess,p->RunProcess?2000:-1,NULL);
				DEBUG_MSG(DEBUG_PLAYER,T("player_base::InputThread::EventWait(p->EventProcess,p->RunProcess?2000:-1,NULL):after"));
				p->WaitForProcess = 0;
			...
			default:
				if (p->BufferMax != p->CurrBufferSize2)
				{
					...
					DEBUG_MSG(DEBUG_PLAYER,T("MicroDrive needs data"));
					p->BufferMax = p->CurrBufferSize2;
				}
		...
		else
		{
			...
			p->InputLocked = 0;
#endif
			DEBUG_MSG(DEBUG_PLAYER,T("player_base::InputThread::EventWait(p->EventRunInput,-1,NULL):before"));
			EventWait(p->EventRunInput,-1,NULL);
			DEBUG_MSG(DEBUG_PLAYER,T("player_base::InputThread::EventWait(p->EventRunInput,-1,NULL):after"));
		}
	}
#if defined(TARGET_ANDROID)
	DEBUG_MSG1(DEBUG_SYS,T("player_base::InputThread::Notify(p,PLAYER_THREAD_JVM,0),ThreadId 0x%x"),ThreadId());
	RemoveThreadEnv();
#endif
	SAFE_END
	return 0;
...
#ifdef MULTITHREAD
	int TID = ThreadId();
	assert(p->ProcessLocked != TID && p->InputLocked != TID);
#endif
...
#if defined(TARGET_ANDROID)
	case PLAYER_ERR:GETVALUE(p->bHasError,int);
#endif
...
static int Set(player_base* p, int No, const void* Data, int Size)
{
...
#if defined(TARGET_ANDROID)
	case PLAYER_ERR:SETVALUE(p->bHasError,int,ERR_NONE);
#endif
	...
	case PLAYER_VOUTPUTID: 
		...
		p->VOutputId = New;
		DEBUG_MSG5(DEBUG_PLAYER,T("player::Set::VOutputId %d,TAG:%c,%c,%c,%c"),
					p->VOutputId,\
					(p->VOutputId&0x000000ff),
					(p->VOutputId&0x0000ff00)>>8,\
					(p->VOutputId&0x00ff0000)>>16,\
	...
	case PLAYER_AOUTPUTID: 
		...
		if (New && !NodeEnumClass(NULL,New)) // old driver removed
			New = p->AOutputIdMax;
		p->AOutputId = New;
		DEBUG_MSG5(DEBUG_PLAYER,T("player::Set::AOutputId %d,TAG:%c,%c,%c,%c"),
					p->AOutputId,\
					(p->AOutputId&0x000000ff),
					(p->AOutputId&0x0000ff00)>>8,\
					(p->AOutputId&0x00ff0000)>>16,\
