...
static int Process(dumpoutput* p, const packet* Packet, const flowstate* State)
{
	if (Packet)
	{
		DEBUG_MSG(DEBUG_VIDEO,T("dumpoutput::Process Enter"));
		...
		if (State->CurrTime != TIME_RESEND)
		{
			...
			DEBUG_MSG(DEBUG_VIDEO,T("dumpoutput::Process Time Ok"));
...
static int UpdateInput(dumpoutput* p)
{
	...
	if (!p->File)
	{
		...
		tchar_t URL[MAXPATH]= T("/sdcard/V.yuv");
#else 
		tchar_t URL[MAXPATH]= T("V.yuv");
#endif
		p->File = StreamOpen(URL,1);
		
		DEBUG_MSG2(DEBUG_VIDEO,T("dumpoutput::UpdateInput dump file %s,p->File %d"),URL,p->File);
...
static int ProcessA(dumpoutput* p, const packet* Packet, const flowstate* State)
{
	if (Packet)
	{
		DEBUG_MSG1(DEBUG_AUDIO,T("dumpoutput::ProcessA:Packet->Length %d"),Packet->Length);
		if (State->CurrTime >= 0 && Packet->RefTime > State->CurrTime + SHOWAHEAD)
			return ERR_BUFFER_FULL;
		...
		if (State->CurrTime != TIME_RESEND)
		{
			...
			if (p->File)
			{
				...
				StreamWrite(p->File,ptr,Packet->Length);
			}
		}
	}
	else
...
static int UpdateInputA(dumpoutput* p)
{
	p->Total = 0;
	p->Dropped = 0;
	if ((p->Format.Type == PACKET_SUBTITLE) ||
	...
	{
#ifdef TARGET_ANDROID
		tchar_t URL[MAXPATH]=T("/sdcard/A.pcm");
#else 
		tchar_t URL[MAXPATH]=T("A.pcm");
#endif
		p->File = StreamOpen(URL,1);

		DEBUG_MSG2(DEBUG_AUDIO,T("dumpoutput::UpdateInputA Path %s,p->File %d"),URL,p->File);
	}

	return ERR_NONE;
}

static int GetA(dumpoutput* p, int No, void* Data, int Size)
{
	int Result = ERR_INVALID_PARAM;
	switch (No)
	{
	...
	case OUT_OUTPUT|PIN_FORMAT:
	case OUT_INPUT|PIN_FORMAT: GETVALUE(p->Format,packetformat); break;
	case OUT_INPUT|PIN_PROCESS: GETVALUE((packetprocess)ProcessA,packetprocess); break;
	case OUT_TOTAL: GETVALUE(p->Total,int); break;
	case OUT_DROPPED: GETVALUE(p->Dropped,int); break;
	case VOUT_CAPS: GETVALUE(0,int); break;
	}
...
static int SetA(dumpoutput* p, int No, const void* Data, int Size)
{
	int Result = ERR_INVALID_PARAM;
	switch (No)
	{
	case OUT_INPUT: SETVALUE(p->Pin,pin,ERR_NONE); break;
	case OUT_INPUT|PIN_FORMAT: SETPACKETFORMAT(p->Format,packetformat,UpdateInputA(p)); break;
	case OUT_TOTAL: SETVALUE(p->Total,int,ERR_NONE); break;
	case OUT_DROPPED: SETVALUE(p->Dropped,int,ERR_NONE); break;
	}
	return Result;
...
static void DeleteA(dumpoutput* p)
{
	if (p->File)
	{
		StreamClose(p->File);
...
static int CreateA(dumpoutput* p)
{
	p->Node.Get = (nodeget)GetA;
	p->Node.Set = (nodeset)SetA;
	return ERR_NONE;
}
...
{
	DEBUG_MSG(DEBUG_VIDEO,T("dumpoutput::DumpOutput_Init"));
