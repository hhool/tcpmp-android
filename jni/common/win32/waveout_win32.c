...
static tick_t Time(waveout* p)
{
	...
	if (p->Play)
	{
		...
		real = Scale(t,p->SpeedTime.Den,p->SpeedTime.Num);
		DEBUG_MSG4(DEBUG_AUDIO,T("waveout_win32:Time:t %d,real %d,p->SpeedTime.Num %d,p->SpeedTime.Den %d"),t,real,p->SpeedTime.Num,p->SpeedTime.Den);
		LeaveCriticalSection(&p->Section);
		return t;
	}	
...
static void ReleaseBuffer(waveout* p,wavebuffer* Buffer,bool_t UpdateTick)
{
	...
	if (UpdateTick)
	{
		...
		DEBUG_MSG2(DEBUG_AUDIO,T("android_waveout::ReleaseBuffer:WaveOutTime: Old %d p->Tick %d"),Old,p->Tick);
...
static void Reset(waveout* p)
{
	...
	DEBUG_MSG(DEBUG_AUDIO,T("android_waveout::Reset"));
	...
	DEBUG_MSG2(DEBUG_AUDIO,T("android_waveout::Process::p->Used %d,p->Waiting %d,"),p->Used,p->Waiting);
...
static void Write(waveout* p, tick_t CurrTime)
{
	DEBUG_MSG2(DEBUG_AUDIO,T("android_waveout::Write Begin CurrTime %d p->Play %d"),CurrTime,p->Play);
	...
	if (p->Play)
	{
		while (p->FillFirst != p->FillLast)
		{
			...
			DEBUG_MSG(DEBUG_AUDIO,T("android_waveout::Write ING"));

			if (!p->Waiting && CurrTime >= 0 && Buffer->EstRefTime >= 0 && Buffer->EstRefTime > CurrTime + SHOWAHEAD)
			{
				DEBUG_MSG2(DEBUG_AUDIO,T("android_waveout::Write break CurrTime %d,Buffer->EstRefTime %d"),CurrTime,Buffer->EstRefTime);
				break;
			}
			...
			{
				DEBUG_MSG5(DEBUG_AUDIO,T("android_waveout::AudioTrackWrite::pthread_mutex_lock(&p->PCMDataLock) CurrTime %d,Buffer->EstRefTime %d,Buffer->Block.Ptr %x,Len %d,p->Waiting %d"),CurrTime ,Buffer->EstRefTime,Buffer->Block.Ptr,p->BufferLength,p->Waiting);
	...
	DEBUG_MSG1(DEBUG_AUDIO,T("android_waveout::Write End CurrTime %d"),CurrTime);
}
	...
	while (Length > 0)
	{
		if (p->FillPos >= p->BufferLength)
		{
			...
			if (p->FillLast)
			{
				...
				Last->Next = Buffer;
				DEBUG_MSG2(DEBUG_AUDIO,T("android_waveout::Send:Last->EstRefTime %d,Buffer->Block.Id %d"),Last->EstRefTime,Buffer->Block.Id);
			}
			else
				p->FillFirst = Buffer;
			DEBUG_MSG2(DEBUG_AUDIO,T("android_waveout::Send::Buffer->RefTime %d,Buffer->Block.Id %d"),Buffer->RefTime,Buffer->Block.Id);
			p->FillLast = Buffer;
			p->FillPos = 0;
...
static int UpdatePCM(waveout* p,const audio* InputFormat)
{
	...
	PCMRelease(p->PCM);
	DEBUG_MSG4(DEBUG_AUDIO,T("android_waveout::UpdatePCM: p->BufferScaledTime %d,p->BufferScaledAdjust %d,p->PCMSpeed %d,p->AdjustedRate %d"),
									 p->BufferScaledTime,p->BufferScaledAdjust,p->PCMSpeed,p->AdjustedRate);

	DEBUG_MSG4(DEBUG_AUDIO,T("android_waveout::UpdatePCM: p->Output.Format.Audio.SampleRate %d,p->Output.Format.Audio.Bits %d,p->Output.Format.Audio.Channels %d, p->Output.Format.Audio.FracBits %d"),\
									 p->Output.Format.Audio.SampleRate,p->Output.Format.Audio.Bits,p->Output.Format.Audio.Channels,p->Output.Format.Audio.FracBits);
...
{
	DEBUG_MSG1(DEBUG_AUDIO,T("android_waveout::Process::p->Used %d"),p->Used);
	...
	if (!Packet)
	{
		...
		DEBUG_MSG1(DEBUG_AUDIO,T("android_waveout::Process::if (!Packet) ::p->Waiting %d"),p->Waiting);
	...
	if (p->Speed.Num==0) // benchmark mode (auto adjust speed)
	{
		...
		DEBUG_MSG3(DEBUG_AUDIO,T("android_waveout::Process::Audio speed:%d length:%d (p->Waiting:%d)"),Speed,Packet->Length,p->Waiting);
		return Send(p,Packet->Data,Packet->Length,Packet->RefTime,State->CurrTime,Speed);
	}

	DEBUG_MSG1(DEBUG_AUDIO,T("android_waveout::Process::State->DropLevel %d "),State->DropLevel);
	if (State->DropLevel)
		return ERR_NONE;

	if (p->Used >= p->BufferLimit)
	{
		DEBUG_MSG2(DEBUG_AUDIO,T("android_waveout::Process::if (p->Used >= p->BufferLimit):p->Used: %d p->BufferLimit %d "),p->Used , p->BufferLimit);
		Write(p,State->CurrTime);
		return ERR_BUFFER_FULL;
	}

	DEBUG_MSG3(DEBUG_AUDIO,T("android_waveout::Process::Send reftime:%d used:%d p->Waiting:%d"),Packet->RefTime,p->Used,p->Waiting);
...
{
	DEBUG_MSG(DEBUG_AUDIO,T("android_waveout::UpdateInput"));
	...
	if (p->Handle)
	{
		DEBUG_MSG(DEBUG_AUDIO,T("android_waveout::UpdateInput::AudioTrackDestroy"));
	...
	if (p->Input.Type == PACKET_AUDIO)
	{
		...
		if (p->Input.Format.Audio.Format != AUDIOFMT_PCM) 
		{
			...
			DEBUG_MSG(DEBUG_AUDIO,T("android_waveout::UpdateInput::ERR_INVALID_DATA !AUDIOFMT_PCM"));
		...
		do
		{
			...
			DEBUG_MSG4(DEBUG_AUDIO,T("android_waveout::UpdateInput:AudioTrackCreate: p->Output.Format.Audio.SampleRate %d,p->Output.Format.Audio.Bits %d,p->Output.Format.Audio.Channels %d,p->Output.Format.Audio.FracBits %d"),\
											 p->Output.Format.Audio.SampleRate,p->Output.Format.Audio.Bits,p->Output.Format.Audio.Channels,p->Output.Format.Audio.FracBits);

			MMResult = waveOutOpen(&p->Handle, WAVE_MAPPER, &p->Format,(DWORD)WaveProc,
									(DWORD)p,CALLBACK_FUNCTION);

DEBUG_MSG1(DEBUG_AUDIO,T("android_waveout::UpdateInput::AudioTrackCreate ret %d"),MMResult);
		...
		if (p->Handle)
		{
			...
			DEBUG_MSG4(DEBUG_AUDIO,T("android_waveout::UpdateInput::UpdatePCM(p,&p->Input.Format.Audio) p->Input.Format.Audio.SampleRate %d,p->Input.Format.Audio.Bits %d,p->Input.Format.Audio.Channels %d,p->Input.Format.Audio.FracBits %d"),
		...
		else
		{
			...
			DEBUG_MSG(DEBUG_AUDIO,T("android_waveout::UpdateInput::ERR_DEVICE_ERROR !AUDIOFMT_PCM"));
...
static void Pause(waveout* p)
{
DEBUG_MSG(DEBUG_AUDIO,T("android_waveout::Pause"));
	p->Pausing = &p->FillFirst;
	waveOutReset(p->Handle);
	p->Pausing = NULL;
DEBUG_MSG2(DEBUG_AUDIO,T("android_waveout::Pause:p->Waiting %d p->Used %d"),p->Waiting,p->Used);
...
static int UpdatePlay(waveout* p)
{
	...
	{
		DEBUG_MSG2(DEBUG_AUDIO,T("android_waveout::UpdatePlay:p->Play FALSE!! p->Waiting %d p->Used %d"),p->Waiting,p->Used);
		if (!p->Waiting)
			p->Tick += Scale(GetTimeTick()-p->TimeRef,p->SpeedTime.Num,p->SpeedTime.Den);
		else
		if (p->Handle)
...
static int TimerSet(void* pt, int No, const void* Data, int Size)
{
	...
	case TIMER_TIME:
		...
		p->Tick = *(tick_t*)Data;
		p->TimeRef = GetTimeTick();
		DEBUG_MSG2(DEBUG_AUDIO,T("android_waveout::TimerSet p->Tick %d,p->TimeRef %d"),p->Tick,p->TimeRef);
		LeaveCriticalSection(&p->Section);
		Result = ERR_NONE;
		break;
	}
...
{
	WAVEOUTCAPS Caps;
	DEBUG_MSG(DEBUG_AUDIO,T("android_waveout::Create"));
...
DEBUG_MSG(DEBUG_AUDIO,T("android_waveout::Create::pthread_mutex_init(&p->Section, NULL);"));
DEBUG_MSG(DEBUG_AUDIO,T("android_waveout::Create::pthread_mutex_init(&p->PCMDataLock, NULL)"));
	return ERR_NONE;
}

static void Delete(waveout* p)
{
	DEBUG_MSG(DEBUG_AUDIO,T("android_waveout::Delete"));
	PacketFormatClear(&p->Input);
	PCMRelease(p->PCM);
