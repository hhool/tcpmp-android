...
{
#ifdef UNICODE
	if (!MultiByteToWideChar(CP_UTF7,0,In,-1,Out,OutLen))
		StrToTcs(Out,OutLen,In);
#else
	WCHAR *Temp;
	long w=MultiByteToWideChar(CP_UTF7,0,(char*)In,-1,NULL,0);
	long TempSize = sizeof(WCHAR)*(w+4);
	Temp = malloc(TempSize);
	if (!MultiByteToWideChar(CP_UTF7,0,In,-1,Temp,TempSize) ||
...
{
#ifdef UNICODE
	if (!WideCharToMultiByte(CP_UTF7,0,In,-1,Out,OutLen,0,0))
		TcsToStr(Out,OutLen,In);
#else
	WCHAR Temp[512];
	if (!MultiByteToWideChar(CP_ACP,0,In,-1,Temp,512) ||
		!WideCharToMultiByte(CP_UTF7,0,Temp,-1,Out,OutLen,0,0))
		TcsToStr(Out,OutLen,In);
#endif
}
...
#else
	WCHAR *Temp;
	long w=MultiByteToWideChar(CP_UTF8,0,(char*)In,-1,NULL,0);
	long TempSize = sizeof(WCHAR)*(w+4);
	Temp = malloc(TempSize);
	if (!MultiByteToWideChar(CP_UTF8,0,In,-1,Temp,TempSize) ||
		!WideCharToMultiByte(CP_ACP,0,Temp,-1,Out,OutLen,0,0))
		StrToTcs(Out,OutLen,In);
	free(Temp);
#endif
