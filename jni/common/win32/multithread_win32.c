...
bool_t EventWait(void* Handle,int Time,void* mute) {DEBUG_MSG(DEBUG_SYS,T("multithread_win32::EventWait")); return WaitForSingleObject(Handle,Time) == WAIT_OBJECT_0; } // -1 = INFINITE
void* EventCreate(bool_t ManualReset,bool_t InitState) { return CreateEvent(NULL,ManualReset,InitState,NULL); }
void EventSet(void* Handle) { DEBUG_MSG(DEBUG_SYS,T("multithread_win32::EventSet")); SetEvent(Handle); }
void EventReset(void* Handle) { ResetEvent(Handle); }
void EventClose(void* Handle) { CloseHandle(Handle); }

int ThreadId() { DEBUG_MSG(DEBUG_SYS,T("multithread_win32::ThreadId"));return GetCurrentThreadId(); }
void ThreadSleep(int Time) { DEBUG_MSG1(DEBUG_SYS,T("multithread_win32::ThreadSleep %d"),Time);Sleep(Time); }
