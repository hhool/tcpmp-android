...
bool_t Context_Init(const tchar_t* Name,const tchar_t* Version,int Id,const tchar_t* CmdLine,void* Application)
{
	...
	if(Context())
	{
		DEBUG_MSG(DEBUG_SYS,T("Context_Init exsit"));
		return 1;
	}
#endif
	p = malloc(sizeof(context));
#if defined(TARGET_ANDROID)
	DEBUG_MSG1(DEBUG_SYS,T("Context_Init context %x"),p);
#endif
	if (!p) return 0;
	...
	OverlaySymbian_Init();
#endif
	OverlaySufaceThumb_Init();
#if defined(DEBUG)||defined(_DEBUG)||defined(DUMP)
	DumpOutput_Init();
#endif
...
void Context_Done()
{
	...
	{
#if defined(TARGET_ANDROID)
		DEBUG_MSG(DEBUG_SYS,T("Context_Done  Context NOT exsit"));
#endif
		return;
	}
...
#endif
#if defined(_WIN32)
	OverlaySubtitle_Done();
#endif
	OverlaySufaceThumb_Done();
#if defined(DEBUG)||defined(_DEBUG)||defined(DUMP)
	DumpOutput_Done();
#endif
...
#if defined(TARGET_ANDROID)
	SetContext(NULL);
	DEBUG_MSG(DEBUG_SYS,T("Context_Done  Context FREE"));
#endif
}
