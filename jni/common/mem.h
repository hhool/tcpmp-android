...
#ifndef __MEM_H
#define __MEM_H
#if defined(__SYMBIAN32__)&& (!defined(NO_PLUGINS) || defined(TARGET_PALMOS))
DLL void* malloc_symbian(size_t);
DLL void* realloc_symbian(void*,size_t);
DLL void free_symbian(void*);
#define malloc(n) malloc_symbian(n)
#define realloc(p,n) realloc_symbian(p,n)
#define free(p) free_symbian(p)
#elif defined(TARGET_ANDROID)&& (!defined(NO_PLUGINS) || defined(TARGET_PALMOS))
DLL void* malloc_android(size_t);
DLL void* realloc_android(void*,size_t);
DLL void free_android(void*);
#define malloc(n) malloc_android(n)
#define realloc(p,n) realloc_android(p,n)
#define free(p) free_android(p)
#elif defined(_WIN32) && (!defined(NO_PLUGINS) || defined(TARGET_PALMOS))
