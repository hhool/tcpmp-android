...
int Format_Send(format_base* p,format_stream* Stream)
{
	...
	if (Stream->Packet.RefTime >= 0 && !Stream->DisableDrop)
	{
		...
		if (CurrTime >= 0)
		{
			...
			if (Diff < -TICKSPERSEC/2 && Stream->Format.Type == PACKET_VIDEO)
			{
				...
				DEBUG_MSG5(DEBUG_FORMAT,T("Send stream:%d size:%d time:%d curr:%d drop:%d make sure buffers and pending packets are processed"),Stream->No,Stream->Packet.Length,Stream->Packet.RefTime,CurrTime,Stream->State.DropLevel);
			...
			{
				DEBUG_MSG5(DEBUG_FORMAT,T("Send stream:%d size:%d time:%d curr:%d drop:%d drop"),Stream->No,Stream->Packet.Length,Stream->Packet.RefTime,CurrTime,Stream->State.DropLevel);
	...
	DEBUG_MSG5(DEBUG_FORMAT,T("format_base::Format_Send:stream ID:%d size:%d time:%d curr:%d drop:%d"),Stream->No,Stream->Packet.Length,Stream->Packet.RefTime,CurrTime,Stream->State.DropLevel);
	...
	if (Result == ERR_BUFFER_FULL)
	{
		DEBUG_MSG1(DEBUG_FORMAT,T("format_base::Format_Send:stream ID:%d BufferFull"),Stream->No);
		return Result;
	}
...
static int Format_Process(format_base* p,processstate* State)
{
	...
	DEBUG_MSG1(DEBUG_PLAYER,T("format_base::Format_Process:Before assert State::Time %d"),State->Time);
	assert(State->Time>=0 || State->Time == TIME_BENCH);
	DEBUG_MSG1(DEBUG_PLAYER,T("format_base::Format_Process:After assert State::Time %d"),State->Time);
	...
	for (No=0;No<p->StreamCount;++No)
	{
		...
		if (Stream->Pin.Node)
		{
			...
			if (i == ERR_SYNCED)
			{
				DEBUG_MSG(DEBUG_FORMAT,T("format_base::Format_Process::if (i == ERR_SYNCED)"));
				// try to calc duration once (after first sync)
				if (p->FirstSync)
				{
					p->FirstSync = 0;
				...
				State->BufferUsedAfter = p->BufferUsed;
				DEBUG_MSG(DEBUG_FORMAT,T("format_base::Format_Process::if (i == ERR_SYNCED) OVER"));
				return i;
			}
...
static int LoadSubtitles(format_base* p,tchar_t* FilePath)
{
	int  Return = ERR_NONE;
	...
	Input = GetStream(FilePath,1);

	DEBUG_MSG1(DEBUG_FORMAT,T("format_base::LoadSubtitles:Stream 0x%x"),Input);
	...
	else
	{
		...
		if (Input->Set(Input,STREAM_URL,FilePath,sizeof(tchar_t)*(tcslen(FilePath)+1)) == ERR_NONE)
		{
			DEBUG_MSG1(DEBUG_FORMAT,T("format_base::LoadSubtitles:Format_LoadSubTitle Input 0x%x"),Input);
...
//!SUBTITL
	...
	DEBUG_MSG1(DEBUG_FORMAT,T("format_base::FindSubtitles:URL %s"),URL);

	SplitURL(URL,NULL,0,Path,TSIZEOF(Path),Name,TSIZEOF(Name),Ext,TSIZEOF(Ext));

	SubExt = LangStr(FOURCC('S','U','B','_'),NODE_EXTS);

	DEBUG_MSG3(DEBUG_FORMAT,T("format_base::FindSubtitles:SubExt %s,Path %s, Name %s"),SubExt,Path,Name);

	Stream = GetStream(Path,1);

	DEBUG_MSG1(DEBUG_FORMAT,T("format_base::FindSubtitles:Stream 0x%x"),Stream);

	if (Stream)
	{
		int Result = Stream->EnumDir(Stream,Path,SubExt,1,&DirItem);

		DEBUG_MSG3(DEBUG_FORMAT,T("format_base::FindSubtitles:EnumDir Result %d,FileName %s,Filesize %d"),Result,DirItem.FileName,DirItem.Size);
		...
		while (Result == ERR_NONE)
		{
			...
			if (DirItem.Size!=-1&&(DirItem.Type == FTYPE_SUBTITLE))
			{
				DEBUG_MSG1(DEBUG_FORMAT,T("format_base::FindSubtitles:Path %s"),FullPath);

				if(!tcsncmp(Name,DirItem.FileName,tcslen(Name)))
				{
					DEBUG_MSG1(DEBUG_FORMAT,T("format_base::FindSubtitles:Path Try this File %s"),FullPath);
					if(LoadSubtitles(p,FullPath)==ERR_NONE)
						break;
				}
			}
...
void Format_PrepairStream(format_base* p,format_stream* Stream)
{
	...
	DEBUG_MSG2(DEBUG_FORMAT,T("format_base::Format_PrepairStream:Stream->ID %d,Stream->Format.Type %s"),Stream->No,
		Stream->Format.Type==PACKET_AUDIO?T("PACKET_AUDIO"):(Stream->Format.Type==PACKET_VIDEO?T("PACKET_VIDEO"):T("PACKET_SUBTITLE")));
