...
static int UpdateInput(codecidct* p)
{
	...
	DEBUG_MSG(DEBUG_VCODEC,T("codecidct::UpdateInput"));
	...
	if (Result != ERR_NONE)
	{
		DEBUG_MSG(DEBUG_VCODEC,T("codecidct::UpdateInput::if (Result != ERR_NONE)"));
		PacketFormatClear(&p->In.Format);
	}
	...
	DEBUG_MSG1(DEBUG_VCODEC,T("codecidct::UpdateInput %d"),Result);
...
static int Process(codecidct* p, const packet* Packet, const flowstate* State)
{
	...
	{
		DEBUG_MSG(DEBUG_VCODEC,T("codecidct::Process::if (p->IDCT.Count<=0 && p->IDCT.Width>0 && p->IDCT.Height>0) return ERR_INVALID_DATA"));
		return ERR_INVALID_DATA;
	}
	...
	{
		DEBUG_MSG(DEBUG_VCODEC,T("codecidct::Process::if (!Packet) IDCT->Null(IDCT,State,0)"));
