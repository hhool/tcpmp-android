...
static int Init(gdi* p)
{
	...
	/*modify*/p->s = NodeEnumObject(0,OVERLAY_SUBTITLE_ID);
	return ERR_NONE;
}
...
static int ProcSub(gdi* p,void* SubInfo)
{	
	memcpy(&p->SubInfo,SubInfo,sizeof(SubtitleInfo));
	return ERR_NONE;
}
...
static int Blit(gdi* p, const constplanes Data, const constplanes DataLast )
{
	...
	if(p->s)
	{
		DrawSubInMem(p->s, &(p->p),&(p->Overlay),&(p->SubInfo),SUBTITLE_GDI, -1);
		DrawSubtitle(p->s,p->Planes[0]);
	}
#endif
