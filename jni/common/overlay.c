...
int OverlayDefaultBlit(overlay* p, const constplanes Data, const constplanes DataLast)
{
	...
	if (Result==ERR_NONE)
	{
		DEBUG_MSG1(DEBUG_VIDEO,T("p->Output.Format.Video.Pitch %d"),p->Output.Format.Video.Pitch);
...
static int Create(overlay* p)
{
	...
	p->ProcSub = (ovlprocsub)OverlayDefaultProcSub;
...
int OverlayUpdateAlign(overlay* p)
{
	...
	DEBUG_MSG4(DEBUG_VIDEO,T("BLIT Viewport:%d %d %d %d"),p->Viewport.x,p->Viewport.y,p->Viewport.Width,p->Viewport.Height);
	...
	DEBUG_MSG4(DEBUG_VIDEO,T("BLIT DstRect:%d %d %d %d"),p->DstAlignedRect.x,p->DstAlignedRect.y,p->DstAlignedRect.Width,p->DstAlignedRect.Height);
	DEBUG_MSG4(DEBUG_VIDEO,T("BLIT SrcRect:%d %d %d %d"),p->SrcAlignedRect.x,p->SrcAlignedRect.y,p->SrcAlignedRect.Width,p->SrcAlignedRect.Height);
...
int OverlayDefaultProcSub(void* This,void* SubDrawInfo)
{
	return ERR_NONE;
}
static int Blit(overlay* p,const packet* Packet,const flowstate* State)
{
	...
	if (Packet)
	{
		if (State->DropLevel)
		{
			DEBUG_MSG(DEBUG_VIDEO,T("overlay::Blit::State->DropLevel"));
			++p->Dropped;
			return ERR_NONE;
		}
		...
		if (State->CurrTime >= 0)
		{
			...
			{
				DEBUG_MSG3(DEBUG_VIDEO,T("overlay::Blit::if (!p->Play %d && State->CurrTime %d == p->LastTime %d) return ERR_BUFFER_FULL"),p->Play,State->CurrTime,p->LastTime);
				return ERR_BUFFER_FULL;
			}

			if (Packet->RefTime >= (State->CurrTime + SHOWAHEAD))
			{
				DEBUG_MSG2(DEBUG_VIDEO,T("overlay::Blit::if (Packet->RefTime %d >= (State->CurrTime %d + SHOWAHEAD) ) return ERR_BUFFER_FULL"),Packet->RefTime,State->CurrTime);
				return ERR_BUFFER_FULL;
			}
		...
		/* { static tick_t Last = 0;
			...
			Last = Packet->RefTime; } */
			DEBUG_MSG2(DEBUG_VIDEO,T("overlay::Blit::p->Blit Packet->RefTime %d,State->CurrTime %d"),Packet->RefTime,State->CurrTime);
...
int OverlayGet(overlay* p,int No,void* Data,int Size)
{
	...
	case OVERLAY_INPUT_SUB_PROCESS:GETVALUE((ovlprocsub)p->ProcSub,ovlprocsub); break;
	}
	return Result;
}
