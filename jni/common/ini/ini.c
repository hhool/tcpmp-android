...
static int                 __ini_close           (ini_t *ini, bool_t flush);
static void                __ini_delete          (ini_t *ini);
static bool_t                __ini_extractField    (ini_t *ini, FILE *file, ini_parser_t *parser, char ch);
static int                 __ini_process         (ini_t *ini, FILE *file, const char *comment);
inline bool_t                __ini_processComment  (ini_t *ini, FILE *file, ini_parser_t *parser);
...
ini_t *__ini_open (const char *name, ini_mode_t mode, const char *comment, int flags)
{
	...
	ini->tmpSection = (struct section_tag*)malloc(sizeof(struct section_tag));
	if (!ini->tmpSection)
		goto ini_openError;
	memset(ini->tmpSection,0,sizeof(struct section_tag));

	ini->tmpKey = (struct key_tag*)malloc(sizeof(struct key_tag));
	if (!ini->tmpKey)
		goto ini_openError;
	memset(ini->tmpKey,0,sizeof(struct key_tag));
    ...
    {   // Process existing ini file
		DEBUG_MSG(DEBUG_INI,T("READ_INI_PROFILE"));
 ...
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
int __ini_close (ini_t *ini, bool_t flush)
{
    FILE *file;
    int   ret = 0;
    ...
    if (ini->tmpSection&&ini->tmpSection->heading)
        free (ini->tmpSection->heading);
	if(ini->tmpSection)
		free(ini->tmpSection);
    
	if (ini->tmpKey&&ini->tmpKey->key)
        free (ini->tmpKey->key);
	if(ini->tmpKey)
		free(ini->tmpKey);
#ifdef INI_ADD_LIST_SUPPORT
 ...
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
bool_t __ini_extractField (ini_t *ini, FILE *file, ini_parser_t *parser, char ch)
{
    switch (ch)
    {   // Check for key value
    case '=':
    ...
    case ']':
        if (parser->state == INI_IN_SECTION)
        {
			...
			DEBUG_MSG1(DEBUG_INI,T("[%s]"),ini->last->heading);
        }
        break;
 ...
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
bool_t __ini_processComment (ini_t *ini, FILE *file, ini_parser_t *parser)
{
        ...
        while (count--)
        {
            ...
            __ini_processDataEnd:
                ...
                if (parser.key)
                {
					...
					parser.key->val   = (char*)malloc(parser.key->length+1*sizeof(char));
					memset(parser.key->val,0,parser.key->length+1*sizeof(char));
					fseek (file, parser.key->pos, SEEK_SET);
					valCount   = fread (parser.key->val, sizeof(char),
                         parser.key->length, file);

					switch(parser.key->val[0])
					{
					case INI_TYPE_INT:
					case INI_TYPE_UINT:
					case INI_TYPE_INT64:
					case INI_TYPE_UINT64:
					case INI_TYPE_FLOAT:
					...
					sscanf(parser.key->val,"%[^:]",buffer);
					parser.key->vtlength = strlen(buffer)+1;//1 ':' 
					parser.key->valtype = parser.key->val[0];
					DEBUG_MSG2(DEBUG_INI,T("%s=%s"),parser.key->key,parser.key->val);
...
int __ini_enumHead(ini_t* ini,int no,char* szHead,int* size)
{
	struct section_tag* selected = NULL;
	int index = 0;
	if (!ini->first)
        return -1;
	...
	selected = ini->first;
    // Go through all sections deleting them
    while(selected&&index<no)
    {
		selected = selected->pNext;
		index++;
    }
	if(index == no && selected != NULL)
	{
		strcpy(szHead,selected->heading);//fix
		return 0;
	}

	return -1;
}
int __ini_enumKey(ini_t* ini,int no,struct key_tag* key)
{
    struct key_tag *selected_k;
	int index = 0;
    if (!ini)
        return -1;
    ...
    if (!ini->selected)
        return -1;
    selected_k = ini->selected->first;
    if (!selected_k)
        return -1;

	while(selected_k&&index<no)
    {
		selected_k = selected_k->pNext;
		index++;
    }
	if(index == no && selected_k != NULL)
	{
		key=selected_k;
		return 0;
	}
...
int __ini_enumKeyName(ini_t* ini,int no,char* szKeyName,int* size)
{
    struct key_tag *selected_k;
	int index = 0;
    if (!ini)
	...
	if(index == no && selected_k != NULL)
	{
		strcpy(szKeyName,selected_k->key);//fix
		return 0;
	}
	
	return -1;
}
int __ini_enumKeyValue(ini_t* ini,int no,char* szValue,int* size)
{
    struct key_tag *selected_k;
	int index = 0;
    if (!ini)
	...
	if(index == no && selected_k != NULL)
	{
		strcpy(szValue,selected_k->val);//fix
		return 0;
	}
	
	return -1;
}
int __ini_enumKeyValueType(ini_t* ini,int no,int* nValueType)
{
	struct key_tag *selected_k;
	int index = 0;
    if (!ini)
	...
	if(index == no && selected_k != NULL)
	{
		*nValueType = selected_k->valtype;
		return 0;
	}
	
	return -1;
}
int __ini_qureyKey(ini_t* ini,const char* szKeyName,struct key_tag** key)
{
	struct key_tag *selected_k=NULL;
	int index = 0;
	...
	while(selected_k)
	{
		if(!strcasecmp(selected_k->key,szKeyName))
			break;
...
int __ini_queryKeyValue(ini_t* ini,const char* szKeyName,unsigned char* szValue,unsigned int* size,unsigned int *ValueType)
{
	struct key_tag *selected_k = NULL;
	
	if(__ini_qureyKey(ini,szKeyName,&selected_k)==0)
	{
		int valtype;

		if(szValue)
		{
			if(selected_k->valtype == INI_TYPE_INT)
			{
				if(selected_k->val)
				{
					sscanf(selected_k->val+selected_k->vtlength, "%d",(int*)szValue);
				}
			}
			else if(selected_k->valtype == INI_TYPE_INT64)
			{
				if(selected_k->val)
				{
					sscanf(selected_k->val+selected_k->vtlength,"%I64d",(int64_t*)szValue);
				}
			}
			else if(selected_k->valtype == INI_TYPE_BINARY)
			{
				if(selected_k->val)
				{
					memcpy(szValue,selected_k->val+selected_k->vtlength,selected_k->length-selected_k->vtlength);//fix me
				}
			}
			else if(selected_k->valtype == INI_TYPE_STRING)
			{
				if(selected_k->val)
				{
					memcpy(szValue,selected_k->val+selected_k->vtlength,selected_k->length-selected_k->vtlength);//fix me
		...
		if(size)
		{
			*size = selected_k->length-selected_k->vtlength;
		}
		if(ValueType)
		{
			*ValueType = selected_k->valtype;
		}
		return 0;
	}
	return -1;
}
int  __ini_setKeyValue(ini_t* ini,const char* szKeyName,unsigned char* szValue,unsigned int* size,unsigned int ValueType)
{
	int ret = 0;

	struct key_tag *selected_k = NULL;
		
	if(ValueType == INI_TYPE_INT)
	{
		ini_writeInt((ini_fd_t)ini, *(int32_t*)szValue);
	}
	else if(ValueType == INI_TYPE_UINT)
	{
		
	}
	else if(ValueType == INI_TYPE_INT64)
	{
		ini_writeInt64((ini_fd_t)ini, *(int64_t*)szValue);
	}
	else if(ValueType == INI_TYPE_UINT64)
	{

	}
	else if(ValueType == INI_TYPE_FLOAT)
	{

	}
	else if(ValueType == INI_TYPE_STRING)
	{
		ini_writeString((ini_fd_t)ini, szValue);
	}
	else if(ValueType == INI_TYPE_DOUBLE)
	{

	}
	else if(ValueType == INI_TYPE_BINARY)
	{
		ini_writeBinary((ini_fd_t)ini,szValue,*size);
	}
	else
	{
		ret = -1;
...
int __ini_store (ini_t *ini, FILE *file)
{
	...
	DEBUG_MSG(DEBUG_INI,T("WRITE_INI_PROFILE"));
    current_h = ini->first;
    while (current_h)
    {
        ...
        {
			DEBUG_MSG1(DEBUG_INI,T("[%s]"),current_h->heading);
            if (fprintf (file, "[%s]\n", current_h->heading) < 0)
        ...
        while (current_k)
        {
            ...
            if (ini_readBinary((ini_fd_t) ini, str, length) < 0)
                goto __ini_storeError;
			if(current_k->valtype != INI_TYPE_BINARY)
			{
				if(fprintf (file, "%s\n", str) < 0)
					goto __ini_storeError;
			}
			...
			{
				int i = 0;
				if(fwrite(str,1,current_k->length,file) < 0 || fprintf (file, "\n") < 0)
					goto __ini_storeError;
			}
		
			DEBUG_MSG2(DEBUG_INI,T("%s=%s"),current_k->key,current_k->val);
...
int INI_LINKAGE ini_enumHead(ini_fd_t fd,int no,char* szHead,int* size)
{
	if (fd)
        return __ini_enumHead ((ini_t *) fd, no, szHead, size);
    return -1;
}

int INI_LINKAGE ini_enumKey(ini_fd_t fd,int no,struct key_tag* Key)
{
	if (fd)
        return __ini_enumKey ((ini_t *) fd, no, Key);
    return -1;
}
int INI_LINKAGE ini_enumKeyName(ini_fd_t fd,int no,char* szKeyName,int* size)
{
	if(fd)
		return __ini_enumKeyName ((ini_t *) fd, no, szKeyName, size);
    return -1;
}
int INI_LINKAGE ini_enumKeyValue(ini_fd_t fd,int no,char* szValue,int* size)
{
	if (fd)
        return __ini_enumKeyValue ((ini_t *) fd, no, szValue, size);
    return -1;
}
int INI_LINKAGE ini_enumKeyValueType(ini_fd_t fd,int no,int* ValueType)
{
	if (fd)
        return __ini_enumKeyValueType ((ini_t *) fd, no, ValueType);
    return -1;
}
int INI_LINKAGE ini_queryKeyValue(ini_fd_t fd,const char* szKeyName,unsigned char* szValue,unsigned int* size,unsigned int *ValueType)
{
	if (fd)
		return __ini_queryKeyValue((ini_t*) fd,szKeyName,szValue,size,ValueType);
	return -1;
}
int INI_LINKAGE ini_setKeyValue(ini_fd_t fd,const char* szKeyName,unsigned char* szValue,unsigned int* size,unsigned int ValueType)
{
	if (fd)
		return __ini_setKeyValue((ini_t*)fd,szKeyName,szValue,size,ValueType);
	return -1;
}
/********************************************************************************************************************
 ...
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
int INI_LINKAGE ini_append (ini_fd_t fddst, ini_fd_t fdsrc)
{
    ...
    {   // Locate heading in the dst
        ...
        {   // Check if data buffer can hold the key
            ...
            if (ini_readBinary (src, data, length) != i)
                goto ini_appendError;
            if (ini_writeBinary (src+current_k->vtlength,dst, length-current_k->vtlength) < 0)
                goto ini_appendError;
            // Move to next key
            current_k = current_k->pNext;
        }
