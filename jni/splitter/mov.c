...
// int32 int64 fixme
static void ReadSTCO64(mp4* p,filepos_t EndOfAtom)
{
	mp4stream* Stream = p->Track;
	int Count;
...
static  mp4atomtable AtomTable[] =
{
	...
	{ FOURCC('w','a','v','e'), ReadSubAtoms },//for QDM2
	{ FOURCC('-','-','-','-'), ReadSubAtoms },
...
static void ReadSubAtoms(mp4* p,filepos_t EndOfAtom)
{
	...
	while (Reader(p)->FilePos < EndOfAtom - 8)
	{
		...
		DEBUG_MSG4(DEBUG_FORMAT,T("mov.c:ReadSubAtoms:Atom:%c%c%c%c"),(char)((Atom.Atom)&0xff),(char)((Atom.Atom>>8)&0xff),(char)((Atom.Atom>>16)&0xff),(char)((Atom.Atom>>24)&0xff));
		...
		for (i=AtomTable;i->Atom;++i)
			if (i->Atom == Atom.Atom)
			{
				...
				if (i->Atom == FOURCC('w','a','v','e'))
				{
					if(p->Track&&p->Track->Stream.Format.Format.Audio.Format == 0x0450)
					{
						i->Func=ReadExtra;
					}
					else
