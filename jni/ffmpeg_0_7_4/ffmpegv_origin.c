...
#define FFMPEG_VIDEO_ORIGIN_CLASS		FOURCC('F','M','V','O')
...
static const codecinfo Info[] = 
{
	...
	{FOURCC('V','P','6','2'),CODEC_ID_VP6,T("FFmpeg VP62"),
		T("vcodec/vp62")},
	{FOURCC('F','L','S','1'),CODEC_ID_FLASHSV,T("FFmpeg Screen Video 1"),
		T("vcodec/fls1")},
	{FOURCC('F','B','M','P'),CODEC_ID_BMP,T("FFmpeg BMP"),
		T("vcodec/fbmp")},
	{FOURCC('F','T','H','E'),CODEC_ID_THEORA,T("FFmpeg Theora"),
		T("vcodec/fthe")},
	{FOURCC('F','V','P','8'),CODEC_ID_VP8,T("FFmpeg VP8"),
		T("vcodec/fvp8,vcodec/fop8")},
	{0},
};
...
static bool_t BuildOutputFormat(ffmpeg_video* p)
{
	...
	if (p->Context->time_base.num > 0)
	{
		...
		DEBUG_MSG1(DEBUG_VIDEO,T("ffmpegv_origin.c:Process:FrameTime %d"),p->FrameTime);
	}
	else
...
static int Process( ffmpeg_video* p, const packet* Packet, const flowstate* State )
{
	...
	DEBUG_MSG1(DEBUG_VIDEO,T("ffmpegv_origin.c:Process:p->Codec.Packet.RefTime %d"),p->Codec.Packet.RefTime);
...
static const nodedef FFMPEGVideo_Origin =
{
	sizeof(ffmpeg_video)|CF_ABSTRACT,
	FFMPEG_VIDEO_ORIGIN_CLASS,
	CODEC_CLASS,
	PRI_DEFAULT,
	(nodecreate)Create,
	NULL,
...
//	REGISTER_DECODER (MPEG2VIDEO, mpeg2video);
    ...
    REGISTER_DECODER (VP8, vp8);// not finish	
	REGISTER_DECODER (FLASHSV, flashsv);
	REGISTER_DECODER (BMP,bmp);
	REGISTER_DECODER (THEORA, theora);
	REGISTER_DECODER (MPEG2VIDEO, mpeg2video);
	NodeRegisterClass(&FFMPEGVideo_Origin);
	...
	for (i=Info;i->Id;++i)
	{
		...
		Def.Class = i->Id;
		Def.ParentClass = FFMPEG_VIDEO_ORIGIN_CLASS;
	...
	StringAdd(1, BMP.Class, NODE_NAME, T("BMP File"));
	StringAdd(1, BMP.Class, NODE_EXTS, T("bmp:V"));
	StringAdd(1, BMP.Class, RAWIMAGE_FORMAT, T("vcodec/fbmp"));
	NodeRegisterClass(&BMP);
}

void FFMPEGV_Origin_Done()
{
	NodeUnRegisterClass(FFMPEG_VIDEO_ORIGIN_CLASS);
	NodeUnRegisterClass(WMVF_ID);
	NodeUnRegisterClass(RBMP_ID);
}
