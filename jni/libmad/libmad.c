...
{
	DEBUG_MSG(DEBUG_ACODEC,T("libmad::UpdateAdjustTime"));
...
{
	DEBUG_MSG(DEBUG_ACODEC,T("libmad::UpdateInput"));
...
{
	DEBUG_MSG(DEBUG_ACODEC,T("libmad::Process"));
	...
	if (Packet)
	{
		...
		DEBUG_MSG1(DEBUG_ACODEC,T("libmad::Process:BufferWrite %d"),Packet->Length);
	...
	for (;;)
	{
		while (mad_frame_decode(&p->Frame, &p->Stream) == -1)
		{
			...
			{
				DEBUG_MSG(DEBUG_ACODEC,T("libmad::Process  ERR_NEED_MORE_DATA"));
			...
			{
				BufferDrop(&p->Buffer);
				DEBUG_MSG(DEBUG_ACODEC,T("libmad::Process:ERR_OUT_OF_MEMORY"));
		...
		if (p->Skip)
		{
			...
			p->Skip--;
			DEBUG_MSG1(DEBUG_ACODEC,T("libmad::Process  p->Skip %d"),p->Skip);
			continue;
		}
		...
		if (p->Codec.Out.Format.Format.Audio.Channels==2 && p->Synth.pcm.channels==1)
		{
			...
			memcpy(p->Synth.pcm.samples[1],p->Synth.pcm.samples[0],	p->Synth.pcm.length * sizeof(mad_fixed_t));
		}
			...
			p->Codec.Out.Format.Format.Audio.Channels != p->Synth.pcm.channels)
		{
			...
			{
				p->FormatSet--;
				DEBUG_MSG1(DEBUG_ACODEC,T("libmad::Process  p->FormatSet %d"),p->FormatSet);
	...
	DEBUG_MSG1(DEBUG_ACODEC,T("libmad::Process END p->Codec.Packet.Length %d"),p->Codec.Packet.Length);
	return ERR_NONE;
}

static int Flush( libmad* p )
{
	DEBUG_MSG(DEBUG_ACODEC,T("libmad::Flush"));
...
{
	DEBUG_MSG(DEBUG_ACODEC,T("libmad::Create"));
